  ...
  <div class="navbar navbar-inverse navbar-fixed-top">
    <div class="container-fluid ">
      <div class="navbar-header">
        ...
        <a href="adminp.php" class="navbar-brand">Quaid-I-Azam University,Islamabad.</a>
      </div>
		...
		?>">Current Time Table</a></li>
          ...
          <li  class="nav-item active dropdown"><a href="" title="Time Table" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Message<span class="caret"></span></a>
            ...
            <li class="dropdown-submenu active">
                ...
                <ul class="dropdown-menu">
                  <li class="nav-item active"><a tabindex="-1" href="template1.php?template=1">Template Class Cancellation Teacher</a></li>
    ...
    <div class="row">
      <div class="col-md-6 col-md-offset-3" id="topPage">
        <form class="padTop"  method="post">
          <?php
               $chk=new SendMessage;
               $chk->sendMessageTeacher();
          ?>
          <?php
            $chk=new AdminPanel;
            $g=$chk->selectTemplate();
            $h=$g['subject'];
            $i=$g['temp_desc'];
          ?>
         <label class="clr" for="course">Enter Teacher Email</label>
          <input type="email" name="mail" title="enter Teacher email here" class="form-control" placeholder="Enter Teacher Email" required>
          <label class="clr1" for="subject">Subject</label>
          <input type="text" name="subject" value="<?php echo $h; ?>" title="enter subject here" class="form-control" placeholder="Enter Subject">
          <label class="clr1" for="subject">Message</label>
          <textarea class="form-control" id="txt"  name="description" style="overflow:auto;" title="enter message here">
            <?php
             echo $i;
            ?>
          </textarea>
          <input type="submit" name="submit" title="Click to Send Message!" value="Send to Teacher" class="btn btn-success btn-lg padTop">  
        </form>
      </div>
    </div>
  </div>
